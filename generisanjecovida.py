...
def generate_pdf():
    ...
    global nazivfajla
    nazivfajla = f"{today_date}_{rezult}_COVID.pdf"
    ...
    danasnjidatum = datetime.date.today().strftime("%d-%m-%Y")
    pol = sys.argv[3]
    protokol = sys.argv[4]
    uzorak = sys.argv[5]
    rezultat = sys.argv[6]
    ...
    ostatak_sadrzaja = [
        ...
        Spacer(1, 5),
        HRFlowable(width="100%", thickness=0.5, lineCap='round', color='black'),
        Paragraph(f"<b>Datum pregleda: </b>  {current_time}", levo),
        Paragraph(f"<b>Broj protokola: </b> {protokol}", levo, encoding="utf-8"),
        Paragraph(f"<b>Uzorak: </b> {uzorak}", levo),
        Paragraph(f"<b>Rezultat: </b>  {rezultat}", levo),
        Paragraph("<b>Virusi: </b>  SARS - COV - 2", levo),
        Paragraph("<b>Napomena: </b>    ANTIGEN ( Ag ) SARS-COV-2 VIRUSA <br></br><br></br> KVALITATIVNA DETEKCIJA Ag ( BRZI IMUNOHROMATOGRAFSKI TEST ) <br></br><br></br>* Rezultat tumačiti u skladu sa kliničkom slikom pacijenta i rezultatima drugih dijagnostičkih procedura.", levo),
        Spacer(1, 5),
